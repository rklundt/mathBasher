...
### Number Climb hero — Space Robot

- **Generator:** Midjourney
- **Files used:**
  - `public/assets/sprites/hero/space-robot.webp` (128×128 lossless WebP with alpha, ~17 KB)
- **Modifications:** Single 128×128 source image (transparent background, arcade-style space robot figure) processed through the `hero` PROFILE (max 192×192 bounding box — source already under, so no resize occurred), metadata stripped (no EXIF / XMP / generator-name leak), encoded as lossless WebP. Original raw output retained locally in `.sprite-source/raw/hero/processed/` as `hero_climber.png` (gitignored) — the source filename was the original signal of intent (the "climber" suffix).
- **Notes:** Added sprint 2.4.1 → corrected in sprint 2.4.2 hotfix. The Space Robot is the **Number Climb** hero (the climber character in the vertical ladder-climb game mode), replacing the long-standing procedural amber-rectangle placeholder used while gameplay landed in sprint 2.2. First time the project lets the player pick a hero — `Settings.heroSkin` (`'space-robot'` default, `'og-yellow'` opt-in for the original amber rectangle) selects at scene mount time. UI lives in Settings → Game → Hero (visible only when the kid is in a Number Climb round). Single static sprite (no animation frames); a multi-frame walking/climbing variant is a future-sprint option if needed. Alien Shoot is unaffected — it continues to use the existing Speeder1/2/3 round-robin.

### Hero Chooser — 4 diverse player avatars

- **Generator:** Midjourney
- **Files used:**
  - `public/assets/sprites/hero/hero-chooser-1.webp` (192×192 lossless WebP with alpha, ~23 KB)
  - `public/assets/sprites/hero/hero-chooser-2.webp` (192×192 lossless WebP with alpha, ~24 KB)
  - `public/assets/sprites/hero/hero-chooser-3.webp` (192×192 lossless WebP with alpha, ~25 KB)
  - `public/assets/sprites/hero/hero-chooser-4.webp` (192×192 lossless WebP with alpha, ~23 KB)
- **Modifications:** Four 512×512 source images (transparent background, dynamic leaping/firing poses in a matching white/teal/orange spacesuit aesthetic) resized to fit within 192×192 bounding box (`hero` PROFILE), metadata stripped (no EXIF / XMP / generator-name leak), encoded as lossless WebP. Original raw outputs retained locally in `.sprite-source/raw/hero/processed/` as `splash_heroChooser_{1,2,3,4}.png` (gitignored).
- **Notes:** Added sprint 2.5 story 4 — the **Hero Chooser** picker. Four diverse heroes covering a female/male × dark/light skin-tone matrix so every kid can see themselves in the character they play. Picker shows on first visit (full-screen `HeroChooserScene`); chosen hero persists via `Settings.chosenHero` (localStorage). The chosen hero appears as: (1) MenuScene top-left avatar (tap to re-open the picker), (2) GameOverScene top-left banner. **Cosmetic only** — does NOT affect in-game sprites (Alien Shoot uses Speeders, Number Climb uses its skin picker, Asteroid Field uses asteroid-heroes round-robin). Filenames are intentionally numeric (not demographic) so analytics and code carry no encoded labels; the visual identity reads from the image itself.
