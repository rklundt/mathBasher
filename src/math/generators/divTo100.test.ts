...
describe('divTo100 generator', () => {
  ...
  describe('1000 sampled questions (seeded)', () => {
    ...
    const samples = Array.from({ length: 1000 }, () => divTo100.generate(rng));

    it('every prompt parses as `dividend ÷ divisor = ?` with divisor,quotient ∈ [2,10] and dividend = divisor × quotient', () => {
      // Use Unicode division sign (U+00F7) — that's what the generator emits.
      const re = /^(\d+) ÷ (\d+) = \?$/;
    ...
    it('every choice is a distinct integer in [2, 10]', () => {
      // Distractor pool IS the quotient range — see divTo100.ts comment.
      for (const q of samples) {
  ...
  it('produces the same sequence given the same seed', () => {
    const a = Array.from({ length: 5 }, () => divTo100.generate(mulberry32(7)));
    const b = Array.from({ length: 5 }, () => divTo100.generate(mulberry32(7)));
    expect(a).toEqual(b);
  });

  // Documented contract: factor-uniform sampling. Each divisor value 2-10
  // should appear roughly 1/9 of the time. Same shape as the multTo100
  // distribution test.
  describe('divisor distribution (uniform over factors)', () => {
    it('each divisor value 2-10 appears within ±20% of the expected 1/9 share', () => {
