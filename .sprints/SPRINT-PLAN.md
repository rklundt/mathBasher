...
| [2.4.1](phase-2/2.4.1-life-rules.md) | Life rules + hero skin — Climb cumulative 3-strike cap + Asteroid Field wrong-shot deterrent (1.5s FIRE cooldown + round cap + visible lives row + visual cooldown lock) + new "Space Robot" hero sprite with a Settings → Game → Hero picker. 4 stories. | Closed (2026-05-23, staging) — hero wiring + asteroid cap scope corrected in 2.4.2 hotfix; corrected before prod. Released 2026-05-23 in v2.5.0 (as part of the combined promotion). |
| [2.4.2](phase-2/2.4.2-hotfix-hero-wiring-and-asteroid-scope.md) | HOTFIX on 2.4.1 — (1) roll back Alien Shoot to its original Speeder1/2/3 random selection (the Space Robot sprite was incorrectly placed there); move Space Robot to Number Climb as the new default climber (replaces the long-standing procedural amber-rectangle placeholder); Settings → Game → Hero picker moves Alien Shoot → Number Climb. (2) Change Asteroid Field's 2-wrong-shot cap from per-ROUND to per-QUESTION (the intent was always per-wave; sprint 2.4.1 implementation was incorrectly cumulative across the 12-question round). 3 stories (2 fixes + close). | Closed — Released 2026-05-23 (v2.5.0) |
| [2.5](phase-2/2.5-splash-and-graphic-intros.md) | Hero Chooser — 4 Midjourney-generated diverse heroes (female/male × dark/light); kid picks one on first run (hard-gated full-screen scene), choice persists via `Settings.chosenHero` localStorage. Chosen hero appears as MenuScene top-left avatar (tap to re-open picker) + GameOverScene banner. Purely cosmetic — does NOT alter in-game sprites. Scope narrowed mid-sprint: splash + intros carried to 2.5.3. | Closed — Released 2026-05-23 (v2.5.0) |
| [2.5.1](phase-2/2.5.1-pnpm-11-upgrade.md) | pnpm 11 upgrade + settings relocation — dev-toolchain maintenance. Migrate pnpm 9.15 → 11.9.0, relocate `overrides` + `onlyBuiltDependencies` from the package.json `pnpm` field to `pnpm-workspace.yaml` (silences the deprecation warning), regenerate the lockfile under pnpm 11. CI auto-picks-up the version via `pnpm/action-setup@v4` reading `packageManager`, so the post-merge CI run is the load-bearing proof. NO app-version bump (toolchain-only; `version` stays 2.5.0). 2 stories. | Closed — Released 2026-06-26 to prod (no git tag; version unchanged) |
| [2.5.2](phase-2/2.5.2-minor-visual-tweaks.md) | Minor visual tweaks — 5 fixes: (1) Hero Chooser subtitle/card overlap, (2) Space Escape difficulty floor-count subtitle covered by buttons, (3) Climb hero 33% larger + bottom-aligned per floor, (4) Alien Shoot mobile URL-bar covers the speeder, (5) slow alien descent 15% on slow+medium tiers. | In Progress (started 2026-06-26) |
| [2.5.3](phase-2/2.5.3-splash-and-intros.md) | Splash screen + per-game-mode graphic intros — carry-in from sprint 2.5's original scope (renumbered from 2.5.1). Boot splash upgrade using Title-Splash art (static WebP, architected for animated-WebP swap later) + per-game-mode intro beat. Several design questions still open from 2.5's conversation; sprint kicks off with a design-discussion story. | Planned |
