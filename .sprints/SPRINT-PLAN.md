...
| [2.4.1](phase-2/2.4.1-life-rules.md) | Life rules + hero skin — Climb cumulative 3-strike cap (kid currently only loses on the 2nd wrong of a SINGLE floor — no climb-wide ceiling on mistakes) + Asteroid Field wrong-shot deterrent (today the kid can rapid-fire through all answers and still pass; proposed: per-wrong-shot timer reduction, per Climb's pattern) + new "Space Robot" hero sprite as the Alien Shoot default with the existing speeder set preserved as a selectable "OG Yellow" alternative. 4 stories. | In Progress (started 2026-05-22) |
