...
| [2.5.1](phase-2/2.5.1-pnpm-10-upgrade.md) | pnpm 10 upgrade + settings relocation — dev-toolchain maintenance. Migrate pnpm 9.15 → 10.x, relocate `overrides` + `onlyBuiltDependencies` from the package.json `pnpm` field to `pnpm-workspace.yaml` (silences the deprecation warning), regenerate the lockfile under pnpm 10. CI auto-picks-up the version via `pnpm/action-setup@v4` reading `packageManager`, so the green PR run is the load-bearing proof. NO app-version bump (toolchain-only; `version` stays 2.5.0). 2 stories. | Planned |
